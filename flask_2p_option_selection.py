...
def page_2_wrap_other_funcs(json_data_raw:dict):
    ...
    def unfoil_outer_json(json_data_whole:dict):
        ...
        dict_ = {travel_item: [''] for travel_item in travel_item_list}
    ...
    def return_select_sqls(where_dict):
        ...
        # 가는 표 오는 표 각각 필요할 듯
        # 1. 항공권
        flight_other_columns = where_dict[travel_item_list[0]][0]
        flight_where_to_jeju = f'arrival == \'CJU\' AND DATE(SUBSTR(departure_datetime, 1, 10)) == \'{where_dict[travel_item_list[0]][1][0]}\''
        flight_where_from_jeju = f'departure == \'CJU\' AND DATE(SUBSTR(departure_datetime, 1, 10)) == \'{where_dict[travel_item_list[0]][1][1]}\''
        ...
        sql_select_cars = f"""
        SELECT *
        FROM {travel_item_list[1]}
        WHERE
        {where_dict[travel_item_list[1]][0]}
        ...
        sql_select_hotels = f"""
        SELECT *
        FROM {travel_item_list[2]}
        WHERE
        {where_dict[travel_item_list[2]][0]}
        ORDER BY ratings DESC
        LIMIT 3
        """
